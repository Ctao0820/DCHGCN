...
#     os.environ['CUBLAS_WORKSPACE_CONFIG'] = ':4096:8'  # 或 ':16:8'。在运行程序前设置环境变量以启用CuBLAS的确定性模式
...
for (FLAG, train_num, Scale) in [(2, 5, 500)]:
# for (FLAG, train_num, Scale) in [(1,5,500),(2,5,500),(3,5,500)]:
    ...
    if FLAG == 1:
        data_mat = sio.loadmat('./datasets//Utopia.mat')
        data = data_mat['Utopia']
        gt_mat = sio.loadmat('./datasets/Utopia_gt.mat')
        gt = gt_mat['Utopia_gt']
        # 参数预设
        ...
        max_epoch = 500  # 迭代次数
        dataset_name = "UP_"  # 数据集名称
        # superpixel_scale=100
        pass
    if FLAG == 2:
        data_mat = sio.loadmat('./datasets/NiliFossae.mat')
        data = data_mat['NiliFossae']
        gt_mat = sio.loadmat('./datasets/NiliFossae_gt.mat')
        gt = gt_mat['NiliFossae_gt']
        # 参数预设

        class_count = 9  # 样本类别数
        learning_rate = 5e-4  # 学习率
        max_epoch = 500  # 迭代次数
        dataset_name = "NF_"  # 数据集名称
        # superpixel_scale = 100
        pass
    if FLAG == 3:
        data_mat = sio.loadmat('./datasets/holden.mat')
        data = data_mat['holden']
        gt_mat = sio.loadmat('./datasets/holden_gt.mat')
        gt = gt_mat['holden_gt']
        # 参数预设
    ...
    color_map_dict = {
        ...
        'HC_': np.array([[255, 0, 0], [0, 255, 0], [0, 0, 255], [255, 255, 0], [255, 0, 255], [0, 255, 255]], dtype=np.uint8),

        'NF_': np.array([[255, 0, 0], [0, 255, 0], [0, 0, 255], [255, 255, 0], [255, 0, 255], [0, 255, 255], [200,100, 0], [0, 200, 100], [100, 0, 200]], dtype=np.uint8),

        'UP_': np.array([[255, 0, 0], [0, 255, 0], [0, 0, 255], [255, 255, 0], [255, 0, 255], [0, 255, 255], [200,100, 0], [0, 200, 100], [100, 0, 200]], dtype=np.uint8),
        ...
        for i, graph in enumerate(hop_graph):
            if len(graph) > K:
                ...
                for neighbor in graph:
                    # 将每对超像素的特征转换为图像格式，并计算其SSIM值
                    img1 = data[i].cpu().numpy()
                    img2 = data[neighbor].cpu().numpy()

                    # 计算 SSIM，返回的值范围是 [-1, 1]，1 表示完全相似
                    ssim_value = ssim(img1, img2, data_range=img2.max() - img2.min())
                    ssim_values.append(ssim_value)

                # 按SSIM值进行排序，选取最大值的K个邻居
                knn_indices = np.argsort(ssim_values)[-K:]
    ...
    for curr_seed in Seed_List:
        ...
        train_label_mask = np.any(train_label_mask.reshape([m * n, class_count]), axis=1).astype(np.float32)
        ...
        test_label_mask = np.any(test_label_mask.reshape([m * n, class_count]), axis=1).astype(np.float32)
        ...
        val_label_mask = np.any(val_label_mask.reshape([m * n, class_count]), axis=1).astype(np.float32)
        ...
        net = model.HDCGNet(height, width, bands, class_count, Q, W_e=W,
        ...
        class FocalLoss(torch.nn.Module):
            ...
            def forward(self, inputs, targets, mask):
                ...
                return torch.sum(focal_loss * mask)

        # compute_loss函数
        def compute_loss(predict, real_label_onehot, real_label_mask):
            real_label_indices = torch.argmax(real_label_onehot, dim=1)  # 转换one-hot为索引
            real_label_mask = real_label_mask.float()  # 确保掩膜是浮点类型
            criterion = FocalLoss(alpha=0.25, gamma=2.0)
            return criterion(predict, real_label_indices, real_label_mask)
        ...
        with torch.no_grad():
            ...
            map[gt == 0, :] = np.array([0, 0, 0], dtype='uint8')
            plt.figure()
            plt.imshow(map, cmap='jet')
            plt.xticks([])
            plt.yticks([])
            plt.savefig(os.path.join("./results/",
                                     'HGCN-MHF-DSC_' + dataset_name + str(train_num) + '_' + str(testOA) + '.png'),
                        bbox_inches='tight',
                        pad_inches=0,  # 移除内边距
                        dpi=400)
